...
wf = wave.open("audio/125bpm_kick16bit.wav", 'rb')
fs = wf.getframerate()
...
def beat_detect(in_data):
    ...
    fft_of_sample = (np.fft.fft(audio)[0:int(len(audio) / 2)]) / len(audio)

    audio_fft = np.abs(fft_of_sample)
    ...
    # Only bass stuff
    bass_indices = [idx for idx, val in enumerate(freqs) if val >= 20 and val <= 250]
    ...
    # Now calculate the time between the beats
